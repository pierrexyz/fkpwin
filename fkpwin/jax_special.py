from jax.numpy import swapaxes, moveaxis, transpose 
...
class Interpolator1D_():
    ...
    def __call__(self, new_x):
        ...
        if new_x.ndim == 1:
            ...
            #check that there are only two dimensions and if not raaise not implmented error 
            if new_x.ndim != 2:
                print(new_x.ndim)
...
class Interpolator1D_():
    ...
    def __call__(self, new_x):
        ...
        if new_x.ndim == 1:
            # Directly call the interpolator for 1D array
            return self.get_1D(new_x)
        elif new_x.ndim == 2:
            return self.get_2D(new_x)
        elif new_x.ndim == 3:
            return self.get_3D(new_x)
        else:
            raise Exception('wrong dimension')

    def get_1D(self,new_x):
        result = self.I1D.__call__(new_x)
        return swapaxes(result, 0, self.interp_axis)

    def get_2D(self,new_x):
        interp_axis_pos = self.interp_axis if self.interp_axis >= 0 else new_x.ndim + self.interp_axis
    ...
    def get_3D(self,new_x):
        temp = new_x.reshape(-1)
        return transpose(self.I1D.__call__(temp).reshape(new_x.shape),(0,1,2))
